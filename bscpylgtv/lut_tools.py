...
if np:
    ...
    from .constants import BT2020_PRIMARIES, DV_PICTURE_MODES, DV_BLACK_LEVEL, DV_GAMMA, DV_CONFIG_FILENAME
...
# Generated by aiopylgtv on {today}
# Display: Unspecified
# DM Version:\x20
"""
...
"""
    ...
    def write_dolby_vision_config(data, version=2019):
        ...
        if type(data) is list and len(data) > 0 and len(data) < 4:
            ...
            for modeData in data:
                if type(modeData) is dict:
                    if ('picture_mode' in modeData
                        and modeData['picture_mode'] in DV_PICTURE_MODES
            ...
            if config != '':
                with open(DV_CONFIG_FILENAME, "w", newline='\r\n') as f:
                    f.write(config)

                return DV_CONFIG_FILENAME
